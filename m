...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 2A3D977877D
	for <lists+linux-arch@lfdr.de>; Fri, 11 Aug 2023 08:33:23 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229543AbjHKGdV (ORCPT <rfc822;lists+linux-arch@lfdr.de>);
        Fri, 11 Aug 2023 02:33:21 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:39980 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230484AbjHKGdU (ORCPT
        <rfc822;linux-arch@vger.kernel.org>); Fri, 11 Aug 2023 02:33:20 -0400
X-Greylist: delayed 609 seconds by postgrey-1.37 at lindbergh.monkeyblade.net; Thu, 10 Aug 2023 23:33:16 PDT
Received: from out-84.mta1.migadu.com (out-84.mta1.migadu.com [95.215.58.84])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id DC1D426B2
        for <linux-arch@vger.kernel.org>; Thu, 10 Aug 2023 23:33:16 -0700 (PDT)
Message-ID: <214aed18-5df5-1014-b73d-a1748c0cca13@linux.dev>
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=linux.dev; s=key1;
        t=1691734984;
        h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
         to:to:cc:cc:mime-version:mime-version:content-type:content-type:
         content-transfer-encoding:content-transfer-encoding;
        bh=0jdrKUOJ0jwShKxNooulXujwDVfYAhim1Trr/gVdmug=;
        b=jUZF3cHDRBDDWgB+km/QAec0oPOP9QqLKgc0q2kpXGnnYB4xdkkDaxqkE+Wh2518zD/pXE
        9bOF2Bnga1UdbQ5ZLHVwS2ycR4UkksH1U4o/sI7/Kgubs8qkdJMnf3D+RNOt27ExEEyQnq
        lP0y6Oow3t46ENl+LyI993L+DcozHqs=
Date:   Fri, 11 Aug 2023 14:22:55 +0800
MIME-Version: 1.0
X-Report-Abuse: Please report any abuse attempt to abuse@migadu.com and include these headers.
From:   Gang Li <gang.li@linux.dev>
Subject: [PATCH v1] docs/zh_CN: add zh_CN translation for memory-barriers.txt
To:     Alex Shi <alex.shi@linux.alibaba.com>, linux-doc@vger.kernel.org
...
X-Spam-Status: No, score=-0.4 required=5.0 tests=BAYES_00,DKIM_SIGNED,
        ...
        SPF_HELO_NONE,SPF_PASS,UPPERCASE_50_75,URIBL_BLOCKED,URI_DOTEDU
...
+
+    如果第一次读的地址与其他 CPU 的写地址重叠，那么其他 CPU 
上写地址之前的写操作，在
+    地址依赖屏障之后都对当前 CPU 可见。当然前提是其他 CPU 
上都是用了写屏障。
+
+     请参阅 "内存屏障序列示例" 一节以查看示意图。
+
+     [!] 请注意，第一个读操作确实需要有一个 _地址_ 
依赖，而不是控制依赖。如果第二个
+ 
读操作的地址依赖于第一个读操作，但依赖性是通过条件而不是实际读地址本身，那么
+     它是一个 _控制_ 
依赖，需要完整的读屏障或更严格的屏障。有关更多信息，请参阅
+     "控制依赖" 一节。
+
+     [!] 请注意，地址依赖屏障通常应与写屏障配对；请参阅 "SMP 屏障配对" 
子节。
+
+     [!] 内核版本 v5.9 删除了显式地址依赖屏障的内核 API。
+     如今，READ_ONCE() 和 rcu_dereference() 
已经包含了地址依赖屏障，无需显式调用。
+
+ (3) 读 (load 或 read) 屏障。
+
+     在系统其他组件看来，所有在屏障之前的 LOAD 操作将在所有屏障之后的 LOAD 
操作之前发生。
+
...
+ (5) ACQUIRE 操作。
+
+     这是单向屏障。它保证对系统的其他组件来说，在 ACQUIRE 
操作之后的所有内存操作都在
+     ACQUIRE 操作之后发生。ACQUIRE 操作包括 LOCK 操作以及 
smp_load_acquire()
...
+     于系统的其他组件来说，看起来是在 RELEASE 操作之前发生。RELEASE 
操作包括
...
+     ACQUIRE 操作之后，在该变量上先前的任何 RELEASE 
操作之前的所有内存访问都保
+ 
证是可见的。换句话说，在给定变量的临界区内，对该变量的所有先前临界区的所有访问
+     都保证已经完成。
+
+     这意味着 ACQUIRE 充当最小的 "acquire" 操作，而 RELEASE 充当最小的 "release" 
操作。
+
+在 atomic_t.txt 中描述的原子操作具有 ACQUIRE 和 RELEASE 
变体，此外还有完全有序
+和松散（无屏障）定义。对于执行读和写的复合原子操作，ACQUIRE 
语义仅适用于读，
+RELEASE 语义仅适用于写。
+
+只有在 CPU 核心之间或 CPU 
和设备之间可能存在交互的情况下，才需要内存屏障。如果代码没有
+多核访问，那么就不需要内存屏障。
+
+
+请注意，这些是 _最低_ 
保证。不同的架构可能会提供更严格的保证，但在特定于架构的代码
+之外，它们可能 _不_ 被依赖。
+
+
+内存屏障做不到什么？
...
+     写屏障只能控制写指令。
+
+ (*) 一个 CPU 上发出内存屏障不会对系统中的另一个 CPU 
或任何其他硬件产生直接影响。
+     内存屏障只能间接地影响其他 CPU 看到该 CPU 的访存顺序，但：
+
+ (*) 即使其他 CPU 正确使用了内存屏障，也不能保证一个 CPU 会正确地看到其他 CPU 
的访存顺序，
+     除非第一个CPU_也_使用了匹配的内存屏障 (请参阅"SMP屏障配对"的子节) 。
+
+ (*) CPU 外的硬件[*]也可能乱序。CPU 缓存一致性机制应在 CPU 
之间传播内存屏障的效果，
+     但可能不是按顺序进行的。
+
...
+			      D = *Q;
+
+[!] READ_ONCE_OLD() 对应于 pre-4.15 内核的 
READ_ONCE()，它并未包含地址依赖屏障。
+
...
+			      WRITE_ONCE(*Q, 5);
+
+因此，对 Q 的读和写无需地址依赖屏障。换句话说，即使没有 READ_ONCE() 
的隐式地址
+依赖屏障，这个结果也是被禁止的：
+
+	(Q == &B) && (B == 4)
...
+以及与这些写操作相关的昂贵缓存未命中。这种模式可以用来记录罕见的错误条件等，CPU 
的自
+然排序可以防止这些记录丢失。
+
+
+请注意，地址依赖提供的排序仅局限于包含它的 CPU。有关更多信息，请参见 "多拷贝原子性" 
部分。
+
+
+地址依赖屏障对于 RCU 非常重要，例如，请参见 include/linux/rcupdate.h
+中的 rcu_assign_pointer() 和 rcu_dereference()。这避免其他 CPU 
读到新指针指向
+的地址的旧值。
+
...
+
+读-读 
控制依赖需要一个完整的读内存屏障，而不仅仅是一个隐式的地址依赖屏障来使其正常工作。
+考虑以下代码：
+
+	q = READ_ONCE(a);
+	<隐式地址依赖屏障>
...
+	}
+
+这将无法产生预期的效果，因为没有实际的地址依赖，而是一个控制依赖，CPU 
可能通过预读拿到
+了变量的旧值。应该这么做：
+
+	q = READ_ONCE(a);
...
+
+控制依赖可以与其他类型的屏障正常配对。
+尽管如此，请注意 READ_ONCE() 和 WRITE_ONCE() 都是必须的!在没有 READ_ONCE() 
的情况下，
+编译器可能将 'a' 的读与其他 'a' 的读合并。没有 
WRITE_ONCE()，编译器可能将'b'的写
+与其他'b'的写合并。任何一种情况都可能对排序产生影响。
+
+更糟糕的是，如果编译器能够证明 (比如) 
变量'a'的值始终不为零，那么它完全有权利通过
...
+	}
+
+如果 MAX 定义为 1，那么编译器知道 (q % MAX) 
等于零，在这种情况下，编译器有权将上述代
+码转换为以下代码：
+
+	q = READ_ONCE(a);
+	WRITE_ONCE(b, 2);
+	do_something_else();
+
+考虑到这种转换，CPU 不需要尊重从变量 'a' 读到变量 'b' 
的写之间的顺序。人们可能会想到
+添加一个 barrier()，但这并没有帮助。条件已经消失，barrier 
也无法恢复它。因此，如果你
...
+	WRITE_ONCE(b, 1);
+
+这个例子强调了确保编译器无法猜测你的代码的重要性。更一般地说，尽管 READ_ONCE() 
确实强制
...
+
+人们可能会认为，因为编译器无法重新排序 volatile 访问，并且还无法将 'b' 
的写入与条件
...
+acquire 屏障与 release 
屏障配对，但两者也可以与其他屏障配对，当然也包括通用屏障。
+写屏障与地址依赖屏障、控制依赖屏障、acquire 屏障、release 
屏障、读屏障或通用屏障配对。
...
+				LOAD *C (读取 B)
+
+如果没有干预，尽管 CPU 1 发出了写屏障，CPU 2 仍可能以随机顺序感知 CPU 1 
上的事件：
+
+	+-------+       :      :                :       :
+	|       |       +------+                +-------+  | CPU 2 感知更新的顺序
...
+				LOAD A
+
+在没有屏障的情况下，尽管CPU 1发出了写屏障，CPU 2 可能会随机感知 CPU 1 
上的事件：
+
+	+-------+       :      :                :       :
+	|       |       +------+                +-------+
...
+
+
+为了更完整地说明这个问题，考虑一下如果代码在读屏障的两侧都包含了读 
A，可能会发生什么：
+
+	CPU 1			CPU 2
+	=======================	=======================
...
+
+
+可以确定的是如果读B的值为2 (B == 2) ，那么第二次读A的值一定为1 (A == 1) 
。对于第一次
...
+
+许多 CPU 
会对读进行预测：当它们检测到它们需要从内存中读某变量，同时没有其他读操作在使用
+总线时，即使指令流还没有执行到那个读，CPU 
也会提前做读操作。这样当真正执行到读操作
+时，指令可以立即完成而无需等待内存相应。
+
+如果预测失败，这条指令实际没有执行，那 CPU 
就会丢弃或将预读到的数据缓存备用。
+
+考虑以下情况：
+
...
+
+不用显式屏障就不保证多拷贝原子性：在这个例子中，CPU 2从X读返回1，CPU 
3从Y读返回1，
+以及从X读返回0都是完全合法的。
+
+关键在于，尽管CPU 2的数据依赖关系对其读和写操作进行了排序，但它并不能确保对CPU 
1的
+写操作进行排序。因此，如果这个程序运行在一个非多拷贝原子系统上，其中CPU 
1和2共享一个
+写缓冲区或一个缓存级别，CPU 2可能会提前访问CPU 
1的写操作。因此，需要通用屏障来确保
+所有CPU在多次访问的组合顺序上达成一致。
+
+通用屏障不仅可以实现多拷贝原子性，还可以生成额外的排序，以确保 -所有- 
CPU都能以相同顺序
+感知到所有操作。相比之下，一系列 发布-获取 
不能提供这种额外的排序，只有链条上的
...
+	r0 == 1 && r1 == 1 && r2 == 1
+
+此外，由于 cpu0() 和 cpu1() 之间的 发布-获取 关系，cpu1() 必须看到 cpu0() 
的写操作，
+因此以下结果不应出现：
+
+	r1 == 1 && r5 == 0
...
+
+尽管 cpu0()、cpu1() 和 cpu2() 
会按顺序看到各自的读取和写入操作，但是未参与发布-获取链
+的 CPU 并不保证能看到这个顺序。这种分歧源于实现 smp_load_acquire() 和
+smp_store_release() 
的弱内存屏障指令，并不要求在所有情况下对先前的写和后续的读进
+行排序。这意味着 cpu3() 可以将 cpu0() 对 u 的写视为发生在 cpu1() 读 v 
-之后-，
...
+
+Linux 
内核具有显式的编译器屏障函数，阻止编译器将其两侧的内存访问移动到另一侧：
+
+	barrier();
+
+这是一个通用屏障 —— barrier() 没有专门的 读-读 或 写-写 
变体。然而，READ_ONCE()
...
函数可以防止许多优化，这些优化尽管在单线程代码中完全安全，
+但在并发代码中可能是致命的。以下是这些类型优化的一些例子：
+
+ (*) 编译器有权重新排序对同一变量的读和写，在某些情况下，CPU 
也有权重新排序对同一变量
+     的读。这意味着以下代码：
+
+	a[0] = x;
...
+	a[1] = READ_ONCE(x);
+
+     简而言之，READ_ONCE() 和 WRITE_ONCE() 为多个 CPU 
访问单个变量提供了缓存一致性。
+
+ (*) 
...
+		do_something_with(tmp);
+
+ (*) 编译器有权重新读变量，例如，编译器可能会优化掉我们之前示例中的变量 
'tmp'：
+
+	while (tmp = a)
+		do_something_with(tmp);
...
+     设当前 CPU 是唯一更新变量 'a' 的 CPU。如果变量 'a' 
是共享的，那么编译器的优化
...
+	}
+
+     没有什么能阻止编译器将 process_level() 
转换为以下内容，实际上，这对单线程代码可能是有益的：
+
+	void process_level(void)
+	{
+		flag = true;
...
+
+     如果中断发生在这两个语句之间，那么
+     interrupt_handler() 可能会收到一个混乱的 msg。使用 WRITE_ONCE() 
防止这种情况：
+
+	void process_level(void)
+	{
...
+	}
+
+     请注意，如果此中断处理程序本身可以被中断，并且被其他也访问 'flag' 和 'msg' 
的
+     事物所中断，例如嵌套中断或 NMI，则需要中断处理程序中也使用 READ_ONCE() 
和
+     WRITE_ONCE()。(请注意，现代 Linux 
内核通常不会发生嵌套中断，事实上，如果中
+     断处理程序返回时启用了中断，您将收到 WARN_ONCE()。)
+
+     编译器可以将 READ_ONCE() 和 WRITE_ONCE() 与 不包含屏障的代码进行乱序。
+
+     这种效果也可以使用 barrier() 实现，但 READ_ONCE() 和 WRITE_ONCE() 
更具选择性：
+     对于 READ_ONCE() 和 WRITE_ONCE()，只控制它自己，而对于 barrier()，
+     编译器必须丢弃所有当前缓存在任何寄存器中的值。当然，编译器还必须尊重 
READ_ONCE()
+     和 WRITE_ONCE() 发生的顺序。
+
+ (*) 编译器可以生成写操作，
...
+
+
+ (*) 防止 "读撕裂" 和 "写撕裂"。单个大型访问被替换为多个较小的访问。例如，假设一架构有 
16 位写指令
+     和 7 位立即数的指令，编译器可能会尝试使用两个 16 位写立即指令来实现以下 
32 位写：
+
+	p = 0x00010002;
+
+     请注意，GCC 
确实使用这种优化，这并不奇怪，因为构建常量然后写它可能需要超过
+     两个指令。因此，这种优化在单线程代码中可能是有益的。最近的一个错误 
(已修复)
...
+	foo2.c = foo1.c;
+
+     因为没有 READ_ONCE() 或 WRITE_ONCE() 以及没有 volatile 
标志，编译器完全有
...
+
+请注意，这些编译器屏障对 CPU 没有直接影响，CPU 可以根据需要对其进行重新排序。如果要对 
CPU
...
+
+     这些是用于原子 RMW 
函数的，这些函数不隐含内存屏障，但代码需要内存屏障。
+     例如，不隐含内存屏障的原子 RMW 函数包括加法、减法、 (失败的) 
条件操作、_relaxed
...
+     dma_wmb() 
确保在设备看到自己被赋予所有权之前，CPU已经将数据写入描述符。
+     dma_mb() 隐含了 dma_rmb() 和 dma_wmb()。
+
+     请注意，dma_*() 屏障不为 MMIO 区域的访问提供任何排序保证。有关 I/O 
访问器和
...
+
+     例如，在对 pmem 区域进行非临时性写操作后，我们使用 pmem_wmb() 
确保写操作完成。
...
+ (*) io_stop_wc();
+
+     对于具有写入组合属性的内存访问（例如，由 ioremap_wc() 返回的那些），CPU 
可能
+     会等待先前的访问与后续的访问合并。当此类等待对性能有影响时，可以使用 
io_stop_wc()
...
+
+此规范是一个 _最低_ 
保证；任何特定架构可能提供更实质性的保证，但在通用代码中应当按照最
+坏情况来考虑。
+
+
+取锁函数
...
+ (*) 读/写信号量
+
+在所有情况下，都有针对每种操作的 "ACQUIRE" (获取) 操作和 "RELEASE" (释放) 
操作
+的变体。这些操作都暗示着特定屏障：
+
+ (1) ACQUIRE 操作的含义：
...
+ (3) ACQUIRE 与 ACQUIRE：
+
+     在另一个 ACQUIRE 操作之前发出的所有 ACQUIRE 操作将在该 ACQUIRE 
操作之前完成。
+
+ (4) ACQUIRE 与 RELEASE：
+
...
+
+     ACQUIRE 
操作的某些锁定变体可能会失败，原因可能是无法立即获取锁，或者在等待
+     锁可用时收到唤醒信号。失败的锁不包含任何类型的屏障。
+
+[!] 注意：锁的 ACQUIRE 和 RELEASE 
只是单向屏障的一个后果是，临界区外的指令可能会渗入临界区内部。
+
+不能假定 ACQUIRE 后跟 RELEASE 是完全内存屏障，因为在 ACQUIRE 
之前发生的访问可能在
...
+
+当 ACQUIRE 和 RELEASE 分别是锁获取和释放时，如果锁的 ACQUIRE 和 RELEASE 
都针对同
+一个锁变量，则只能从未持有该锁的另一个 CPU 
的角度来看，可以发生这种重新排序。
+简而言之，ACQUIRE 后跟一个 RELEASE 不等于通用屏障。
+
+同样，RELEASE 后跟 ACQUIRE 也不是通用内存屏障。RELEASE 和 ACQUIRE 
可以交叉：
+
+	*A = a;
+	RELEASE M
...
+
+在 UP 
编译的系统上，锁和信号量可能无法提供任何排序保证，因此在这种情况下不能依赖
...
+-------------------------
+
+禁用中断 (相当于 ACQUIRE) 和启用中断 (相当于 RELEASE) 
的函数将仅充当编译器屏障。
...
+
+重复一遍，如果实际唤醒了目标任务，wake_up() 
保证执行通用内存屏障，如果不需要唤醒目标任
...
+	LOAD Y				LOAD X
+
+如果唤醒确实发生，两个读中的至少一个必须看到 
1。另一方面，如果唤醒没有发生，两个加
+载都可能看到 0。
+
+wake_up_process() 
总是执行通用内存屏障。屏障发生在访问任务状态之前。也就是说，如
+果在前面的片段中将 wake_up() 替换为对 wake_up_process() 的调用，那么两个读中的一个将保证看到 
1。
+
+可用的唤醒函数包括：
+
...
+
+[!] 请注意，睡眠和唤醒函数的内存屏障 _不会_ 
在唤醒之前按顺序对多个写进行排序，
+也不会在睡眠者调用 set_current_state() 
之后对读进行排序。例如，如果睡眠者执行以下操作：
+
...
+	wake_up(&event_wait_queue);
+
+不能保证睡眠者会将 event_indicated 的更改视为在 my_data 
的更改之后发生。在这种情
...
+---------------------------
+
+考虑以下情况：系统具有一对自旋锁 (M) 和 (Q) 
，以及三个CPU。然后，如果发生以下事件序列：
+
+	CPU 1				CPU 2
+	===============================	===============================
...
+
+那么除了单独的CPU上单独的锁对CPU 3施加的约束之外，没有保证CPU 
3将看到*A至*H的访问顺序。例如，它可能看到：
+
...
+
+以 R/W 
信号量的慢速路径为例。在这里，一个等待的进程被排队在信号量上，它的堆栈上有
+一部分与信号量的等待进程列表相连接：
+
+	struct rw_semaphore {
+		...
...
+	--- 错误 ---
+
+可以使用信号量锁来处理这个问题，但是这样的话，在被唤醒后 down_xxx() 
函数就不必再次获得自旋锁了。
+
...
+后的所有内存访问之前发生的。它并 _不_ 
保证在屏障指令本身完成时，屏障之前的所有内存访问
+都将完成。
+
+在一个UP系统中 - 在这里这不会成为问题 - smp_mb() 
只是一个编译器屏障，因此确保编译器
...
+-----------------
+
+许多设备可以进行内存映射，因此对 CPU 
来说，访问它们就像访问内存一样。要控制这样的设备，
+驱动程序通常必须以完全正确的顺序进行内存访问。
+
+然而，拥有聪明的 CPU 
或聪明的编译器会产生一个潜在的问题，即驱动代码中仔细安排的访问
+顺序如果没有按照所需的顺序到达设备，CPU 
或编译器认为重新排序、组合或合并访问更有效的
+话，就会导致设备故障。
+
+在 Linux 内核中，I/O 应该通过适当的访问例程 (如 inb() 或 writel()) 
完成，这些例
+程知道如何使此类访问适当地顺序进行。在大多数情况下，这使得显式使用内存屏障变得不必要，
+但是，如果访问函数用于引用具有宽松内存访问属性的 I/O 内存窗口，则需要 _强制性_ 
内存
+屏障来强制排序。
+
...
+备的控制或访问。
+
+至少在某种程度上，可以通过禁用本地中断 (一种锁定形式) 
来缓解这种情况，这样关键操作
+都包含在驱动程序中的禁用中断部分内。在驱动程序的中断例程执行期间，驱动程序的核心可
+能无法在同一 CPU 
上运行，而且在处理当前中断之前，不允许再次发生中断，因此中断处理程
+序不需要对其进行锁定。
+
...
+
+通常这不会是问题，因为在这些部分内完成的 I/O 访问将包括对严格排序的 I/O 
寄存器的同步
+读操作，这些操作形成了隐式的 I/O 屏障。
+
+类似的情况可能发生在中断例程和在单独的 CPU 
上运行的两个相互通信的例程之间。如果可能出
...
+
+通过 I/O 
与外围设备进行交互，在不同架构和设备间有很大的差异。因此，不打算移植的驱动程序
...
+	对于具有默认 I/O 属性的映射指针 (例如，由 ioremap() 返回的指针) 
，排序保证如下：
+
+	1. 所有针对同一外设的 readX() 和 writeX() 访问彼此有序。这确保了同一 CPU 
线
+	   程对特定设备的 MMIO 寄存器访问将按程序顺序到达。
+
+	2. 一个持有自旋锁的 CPU 线程发出的 writeX() 将在另一个 CPU 
线程在后续获取相
+	   同自旋锁后发出的对同一外设的 writeX() 
之前排序。这确保了在持有自旋锁时发出
+	   的对特定设备的 MMIO 寄存器写入将按照获取锁的顺序到达。
+
+	3. 一个 CPU 线程对外设的 writeX() 
将首先等待由同一线程发出或传播给同一线程的
+	   所有先前对内存的写入完成。这确保了 CPU 对由 dma_alloc_coherent() 分配的
+	   出站 DMA 缓冲区的写入在 CPU 写入其 MMIO 控制寄存器以触发传输时对 DMA 
引擎
+	   可见。
+
+	4. 一个 CPU 线程从外设读取的 readX() 
将在相同线程的任何后续内存读取开始之前完
+	   成。这确保了 CPU 从 DMA 引擎的 MMIO 状态寄存器读取以确定 DMA 
传输已完成后
+	   从由 dma_alloc_coherent() 分配的传入 DMA 缓冲区不会看到过时的数据。
+
+	5. 一个 CPU 线程从外设读取的 readX() 将在相同线程上的任何后续 delay() 循环
+	   开始执行之前完成。这确保了 CPU 对外设的两个 MMIO 寄存器写入至少相隔 
1us，
+	   如果第一个写入立即使用 readX() 读回并在第二个 writeX() 之前调用 
udelay(1)：
+
...
+
+	由于许多 CPU 架构最终通过内部虚拟内存映射访问这些外设，因此 inX() 和 
outX()
+	提供的可移植排序保证与分别访问具有默认 I/O 属性的映射时 readX() 和 
writeX()
+	提供的保证相同。
+
+	设备驱动程序可能期望 outX() 发出一个非传递写事务，在返回之前等待来自 I/O 
外
+ 
...
+ (*) insX(), outsX():
+
+	如上所述，分别访问具有默认 I/O 属性的映射时，insX() 和 outsX() 
访问器提供与
...
+	readX()/writeX()。
+
+除了字符串访问器 (insX()、outsX()、readsX() 和 writesX()) 
之外，上述所有内容都
...
+不同架构的内存一致性模型不同，因此我们要选择一致性最差的CPU作为模型，即 DEC 
Alpha
+
+这意味着必须假设 CPU 
将以任何顺序执行指令，甚至可能是并行执行。如果指令流中的一个指令
+依赖于较早的指令，则被依赖的较早指令满足该指令的条件[*]后才能执行这条指令。
+换句话说：要保持指令间的因果关系。
+
+ [*] 一些指令具有多种效果 - 例如改变条件代码、改变寄存器或改变内存 - 
不同的指令可能依
+     赖于不同的效果。
+
+CPU 
可能丢弃没有效果的指令。例如，如果两个相邻的指令都将立即数读到同一个寄存器中，
+那第一个指令可能会被丢弃。
+
+
...
+============================
+
+被缓存的内存的操作在系统中的感知方式在一定程度上受到位于 CPU 
和内存之间的缓存以及维护系统状
+态一致性的内存一致性系统的影响。
+
+就 CPU 通过缓存与系统其他部分交互的方式而言，内存系统必须包括 CPU 
的缓存，而内存屏
...
+	                          :
+
+有的读或写操作可能实际上没有在发出它的 CPU 外部出现，因为它可能已经在 CPU 
自己
+的缓存中得到满足，但就其他 CPU 
而言，它仍然会表现得好像已经发生了完整的内存访问，因为缓存
+一致性机制将在缓存间传播内存操作。
+
+CPU 
核心可以以它认为合适的任何顺序执行指令，前提是保持预期的程序因果关系。一些指令生成读
+和写操作，然后进入要执行的内存访问队列。核心可以按照它希望的任何顺序将这些放入队列中，并继
+续执行，直到它被迫等待指令完成。
+
+内存屏障关心的是控制访问从 CPU 
端到内存端的顺序，以及系统中其他观察者感知到的效果发生的顺序。
+
+[!] 在给定的 CPU 内部，不需要内存屏障，因为 CPU 
总是会看到它们自己的读和写操作，就好像
+它们是按照程序顺序发生的一样。
+
+[!] MMIO 
或其他设备访问可能会绕过缓存系统。这取决于通过哪个内存窗口访问设备的属性和/或
...
+
+另外, 
设备通过DMA写到内存，内存中的新数据可能会被写回的脏cache覆盖。也可能会因为cache没
...
+--------------------------
+
+DEC Alpha CPU 是内存一致性最差的 CPU 之一。不仅如此，某些版本的 Alpha CPU 
具有分割
+的数据cache，这两个 cache 
间没有地址依赖机制，是唯一需要使用地址依赖屏障的地方。
+
...
+----------------------
+
+即使客户机本身编译为不支持 SMP，也可能会受到宿主机 SMP 
效应的影响，从而在与宿主机交互
+时发生错误。在UP客户机与宿主机交互时，可以用强制屏障，但并不是最优解。
+
+内核提供了低级别的 virt_mb() 等宏。即使客户机编译为 UP，它们也会生成与编译为 
SMP 时的
+smp_mb() 等效的代码。客户机应在与 (可能是 SMP 的) 主机同步时使用 virt_mb() 
而不是
+smp_mb()。
+
+在所有其他方面，它们等同于 smp_mb() 等，特别是，它们不控制 MMIO 效果：要控制 
MMIO 效果，
+请使用强制屏障。
+
+
...
+==========
+
+Alpha AXP 架构参考手册，第二版 (Sites & Witek，Digital Press)
+	第 5.2 章：物理地址空间特性
+	第 5.4 章: cache 和写缓冲区
+	第 5.5 章：数据共享
+	第 5.6 章：读/写排序
...
+	第 7.1 章：内存访问排序
+	第 7.4 章：buffer 和组合内存写入
+
+ARM 架构参考手册 (ARMv8，用于 ARMv8-A 架构)
+	第 B2 章：AArch64 应用级内存模型
+
+IA-32 英特尔架构软件开发者手册，第 3 卷：系统编程指南
+	第 7.1 章：锁原子操作
+	第 7.2 章：内存排序
+	第 7.4 章：序列化指令
+
...
+	附录 J：使用内存模型进行编程
+
+PowerPC 中的存储 (Stone 和 Fitzgerald)
+
+UltraSPARC 程序员参考手册
+	第 5 章：内存访问和可缓存性
+	第 15 章：Sparc-V9 内存模型
+
+UltraSPARC III Cu 用户手册
+	第 9 章：内存模型
+
+UltraSPARC IIIi 处理器用户手册
+	第 8 章：内存模型
+
+UltraSPARC 架构 2005
+	第 9 章：内存
+	附录 D：内存模型的正式规范
+
+UltraSPARC T1 补充 UltraSPARC 架构 2005
+	第 8 章：内存模型
+	附录 F：cache 和 cache 一致性
+
+深入 Solaris，核心内核架构，第 63-68 页：
+	第 3.3 章：锁和同步的硬件考虑因素
+
+给内核程序员写的 Unix 系统现代架构，对称多处理和缓存：
+	第 13 章：其他内存模型
+
+英特尔 Itanium 架构软件开发者手册：第 1 卷：
+	第 2.6 节：推测
+	第 4.4 节：内存访问
-- 
2.20.1
